...
class RoleViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = Role.objects.select_related('college', 'created_by')

        # Filter by college if user is not superuser
        if not self.request.user.is_superuser:
            queryset = queryset.filter(
                Q(college=self.request.user.college) | Q(college__isnull=True)
            )
        ...
        if college_id:
            queryset = queryset.filter(Q(college_id=college_id) | Q(college__isnull=True))
